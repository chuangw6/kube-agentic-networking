...
func translateHTTPRouteToEnvoyRoutes(
	...
	authPolicyLister agenticlisters.AuthPolicyLister,
	backendLister agenticlisters.BackendLister,
) ([]*routev3.Route, []*agenticv1alpha1.Backend, metav1.Condition) {

	var envoyRoutes []*routev3.Route
	var allValidBackends []*agenticv1alpha1.Backend
...
func buildHTTPRouteAction(namespace string,
	...
	referenceGrantLister gatewaylistersv1beta1.ReferenceGrantLister,
	authPolicyLister agenticlisters.AuthPolicyLister,
	backendLister agenticlisters.BackendLister) (*routev3.RouteAction, []*agenticv1alpha1.Backend, error) {

	weightedClusters := &routev3.WeightedCluster{}
	var validBackends []*agenticv1alpha1.Backend

	for _, httpBackendRef := range backendRefs {
		backend, err := fetchBackend(namespace, httpBackendRef.BackendRef, backendLister, serviceLister, referenceGrantLister)
		if err != nil {
			return nil, nil, err
		}
		...
		clusterWeight.TypedPerFilterConfig, err = buildPerClusterRBACFilterConfig(authPolicyLister, backend)
		if err != nil {
...
func buildPerClusterRBACFilterConfig(authPolicyLister agenticlisters.AuthPolicyLister, backend *agenticv1alpha1.Backend) (map[string]*anypb.Any, error) {
	...
	rbacConfig, err := rbacConfigFromAuthPolicy(authPolicyLister, backend)
	if err != nil {
